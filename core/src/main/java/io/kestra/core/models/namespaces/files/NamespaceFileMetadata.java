...
public class NamespaceFileMetadata implements DeletedInterface, TenantInterface, HasUID {
    ...
    @JsonCreator
    public NamespaceFileMetadata(String tenantId, String namespace, String path, Integer version, Long size, Instant created, @Nullable Instant updated, boolean deleted) {
        this.tenantId = tenantId;
        this.namespace = namespace;
        this.path = path;
        String withoutTrailingSlash = path.endsWith("/") ? path.substring(0, path.length() - 1) : path;
        this.parentPath = withoutTrailingSlash.contains("/") ?
